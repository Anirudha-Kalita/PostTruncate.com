 ...
 * public/og.webp. The card image is independent from the post "Add media"
 * attachment, so it defaults here when the user has not set a custom card
 * image. Imported by Dashboard and PlatformCounter.
 */
export const DEFAULT_CARD_IMAGE = '/og.webp';

/**
 * Responsive `srcset` for the DEFAULT card image. These WebP variants are
 * generated by `scripts/optimize-images.mjs` (the `og.webp` brand-image ladder)
 * and always exist because the optimizer runs in `predev` and `prebuild`. The
 * widths here MUST match that ladder ([160, 400, 800, 1200]). The browser picks
 * a device-appropriate size instead of always downloading the full image — a
 * 2.8KB 160px file for the editor thumbnail, ~24KB 800px for the live card, etc.
 * Used only for the default image; a user-uploaded blob URL has no variants and
 * renders as a plain <img src>.
 */
export const DEFAULT_CARD_IMAGE_SRCSET = [
  '/og/optimized/og-160.webp 160w',
  '/og/optimized/og-400.webp 400w',
  '/og/optimized/og-800.webp 800w',
  '/og/optimized/og-1200.webp 1200w',
].join(', ');

/** `sizes` hint for the full-width card image (live preview surface). */
const CARD_IMAGE_SIZES = '(max-width: 640px) 90vw, 500px';
...
function CardMedia({
  ...
  ratio: number;
  alt: string;
  /** Responsive srcset (default image only); omitted for user blobs/video. */
  srcset?: string;
  sizes?: string;
  class?: string;
}) {
...
export function LivePreviewCard({
  ...
  // The default OG image has a generated WebP variant ladder, so the card
  // downloads a device-appropriate size (full srcset). A user-uploaded blob URL
  // (or any video) has no variants → plain <img src>, browser scales it.
  const useResponsiveDefault = image === DEFAULT_CARD_IMAGE && mediaKind === 'image';
  const imageSrcset = useResponsiveDefault ? DEFAULT_CARD_IMAGE_SRCSET : undefined;
  ...
  if (layout.imageStyle === 'embed') {
    ...
    return (
      ...
      >
        {image && (
          <CardMedia image={image} mediaKind={mediaKind} ratio={imageRatio} alt={imageAlt} srcset={imageSrcset} sizes={CARD_IMAGE_SIZES} />
        )}
        <div class="p-3">
