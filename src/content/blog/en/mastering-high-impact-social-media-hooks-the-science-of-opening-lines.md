...
The content isn't the problem. The hook is.

Or more specifically: the hook never got a fair shot. Because on mobile LinkedIn, the feed collapses your post after roughly 140 characters. That opening line - the one you thought was leading somewhere - got clipped mid-sentence. The reader saw "Last year I learned something about LinkedIn strateg..." and kept scrolling.

This happens on every platform. And it almost never gets talked about when people teach hook writing.

So this is my attempt to fix that.

## Your hook is dying before anyone reads it

Every major platform truncates posts. That's not a bug - it's a deliberate design choice to pack more content into the feed. But it means that your hook has a much smaller window than you think.

Here's what the fold actually looks like across platforms, according to [PostTruncate's platform limits data](/en/platform-limits/):
...
> "If you don't give your brain time to catch up with your impulses, you just keep scrolling." - [Aza Raskin, inventor of the infinite scroll](https://medium.com/@ruchakher/unfinished-addictive-how-the-zeigarnik-effect-hooks-us-on-social-media-9305c4eca3cd)

The best textual pattern interrupts are structural: a one-word opening sentence, a question that sounds absurd, a declarative statement that's counterintuitive, a line that breaks conventional wisdom. They don't read like the other 40 posts in the feed.

### Negativity bias: problems stop scrolls
...
### Curiosity gaps: the itch the reader has to scratch

A curiosity gap is a hook that promises information without delivering it yet. It creates a gap between what the reader knows now and what the hook suggests they're about to learn - and that gap is uncomfortable. The reader fills it by tapping "see more."
...
**An open loop or unresolved tension.** This is the Zeigarnik Effect in practice. Don't resolve the hook in the hook. Lead the reader to the threshold and stop.

**Authentic confidence.** Hedged language kills hooks. "This might be interesting depending on your situation" is not a hook. A hook has a point of view. It says something without immediately qualifying it to death. Confidence reads as authority. Authority earns attention.

**Fit with the visible window.** This is the one most hook writing advice forgets: the hook needs to work within whatever the platform actually shows before the fold. A brilliant 200-character hook that gets cut at 140 is a 140-character hook - and it had better work as a truncated thought, not a half-finished sentence.
...
-   "I've been posting on LinkedIn for six years. I got more wrong than right for the first four."
...
Specific numbers carry disproportionate weight because they signal research. "Many posts" is an assertion. "94% of posts" is a finding. Even when the reader doesn't know the source, specificity reads as credibility. When the data is also surprising or counterintuitive, it functions as a curiosity gap at the same time.
...
## Platform-by-platform hook strategy

The formulas above apply across all platforms, but how you deploy them changes significantly depending on where you're posting. Platform norms, character limits, and audience expectations all shape what "a good hook" actually looks like.
...
-   A single bold declarative statement followed by a line break and the first supporting point
-   A short personal story opening (2-3 sentences max before the fold)
-   Data-driven opening that's specific enough to surprise

What rarely works:

-   Questions as openers - LinkedIn's professional audience has become immune to the "Agree?" format
-   Generic warm-ups ("Great question I've been thinking about recently")
-   Hooks that only make sense with context that comes later in the post

### X/Twitter

The entire post is effectively the hook on X. With a [280-character hard cap](/en/blog/twitter-x-seo-guide-maximize-post-reach/) - and no "see more" fold - every character is visible, which means the hook dynamic is reversed: rather than compelling a click to "see more," you're competing for engagement on what's already fully visible.

The formulas that work here are the ones with the sharpest edges: counter-narratives, bold data, and short story leads. The [X SEO guide covers this in detail](/en/blog/twitter-x-seo-guide-maximize-post-reach/), but the core principle is that specificity and confidence win. A vague, hedged take gets skipped; a sharp, specific, slightly risky take gets quoted.

For threads, the first tweet is the hook for the entire thread. If you're threading content, the opening needs to establish both the promise and the tone. A good thread hook opens a loop big enough that readers feel compelled to see it closed. The [X thread tools guide](/en/blog/best-x-thread-tools-how-to-automatically-split-and-format-long-tweets/) covers how to structure long content for X specifically.
...
[Facebook's 110-character mobile fold](/en/blog/facebook-post-character-limits-by-device-type/) is the tightest of any platform. That's roughly one medium-length sentence. The fold is ruthless and inconsistently discussed in hook-writing content.

The fear hook and curiosity gap both perform on Facebook because they create enough tension in a single sentence to earn the "see more" tap. The demographic skews older than Instagram or TikTok, which means credibility signals and story-based openings often outperform trend-aware or pop-culture references.

## Mistakes that kill otherwise good hooks

I've made all of these. The list exists because I've seen smart writers - people who clearly understand hook writing at a theoretical level - fall into the same traps.

**Resolving the hook in the hook.** "Here are the 7 things I learned about LinkedIn reach (and they all come down to your opening sentence)" is not a hook with a payoff - it's the entire article in the first line. Leave the loop open. Don't answer your own question.
...
## How to test your hooks before posting

The single most reliable improvement I've made to my hook-writing process is checking how posts actually look on each platform before publishing - not how they read in my notes app.
...
-   The fold line is different on mobile and desktop for most platforms - you can see both
-   You can test the same hook across all six platforms simultaneously, which is useful if you cross-post
-   There's a weighted character counter for X that accounts for t.co link costs (23 characters per URL regardless of actual length), emoji weighting (2 characters each), and CJK character doubling
-   Thread splitting is automatic - if you're writing an X thread, it splits at natural sentence boundaries, not mid-clause

The tool runs entirely client-side. Your text never leaves the browser. There's no account required, no paywall, no credit card.
...
If there's one thing this post is trying to land, it's this: a good hook that gets cut at the fold is a bad hook. The writing quality doesn't matter if the first visible characters don't earn the "see more" tap.

[PostTruncate](/en/character-counter/) exists for exactly this problem - it shows you where your post gets cut on every major platform, in real time, before you publish. You can see the fold line for LinkedIn's 140-character mobile collapse, Facebook's brutal 110-character cut, Instagram's 125-character threshold, and Threads' 250-character window all at once. Write your hook, run it through the preview, adjust until it survives the fold. No sign-up, no cost, no tracking.
