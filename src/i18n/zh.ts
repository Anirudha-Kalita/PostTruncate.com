...
export const zh: Translations = {
  ...
  seoCopy: {
    ...
    sections: [
      ...
      {
        heading: '高级短信字符计数器和分段计算器',
        paragraphs: [
          '专业的<strong>短信字符计数器</strong>远远超越基本的文本长度检查，能够实时分析消息底层的电信数据结构。该工具专为复杂的消息工作流而打造，可以准确区分标准的 <strong>GSM 7 位编码</strong>（每条短信最多允许 160 个字符）和 <strong>Unicode 编码</strong>（将上限降至 70 个字符）。',
          '此外，它还考虑了导致运营商意外计费的隐藏技术细节。例如，它会追踪 <strong>GSM 扩展表字符</strong>——如欧元符号 (€)、方括号 [ ]、花括号 { } 和竖线符号 (|)——这些字符可以安全地保留在 GSM-7 模式中，但每个会立即占用 2 个字符位。',
          '如果你的营销文案超过单条短信的阈值，我们内置的<strong>短信分段计算器</strong>会估算精确的多段消息拆分数量，并将不可见的用户数据头 (UDH) 计算在内——UDH 会将多段消息的边界缩减为 GSM 的 153 个字符或 Unicode 的 67 个字符。',
        ],
      },
      {
      ...
      },
      {
        ...
        paragraphs: [
          ...
          '通过分析你的<strong>含空格字符计数器</strong>并评估你的<strong>文本中的字符数</strong>等参数，你可以把文案的排版做到尽善尽美。此外，仪表板还会实时监测你的话题标签密度。如果你的资料布局中话题标签超过 5 个，我们的系统会立即发出警告，保护你的账号免遭算法的意外限流。它是一款集大成的<strong>字词与字符计数器</strong>和<strong>在线字数统计工具</strong>，助力社交增长。',
        ],
      },
      {
  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a: '这是由于您文本的编码方式发生了变化。标准短信使用GSM 7位编码，每个SMS数据包最多允许160个字符。但是，只要您插入一个非GSM字符——例如表情符号、特殊符号或地区文字（如阿萨姆语或印地语）——整条消息就会立即强制切换为Unicode编码。当消息切换为Unicode时，每个SMS数据包的最大容量会从160个字符急剧下降到仅70个字符。此外，如果您的文本超过了70个字符的阈值，系统必须使用多部分拼接数据（用户数据标头），这会将每个拼接SMS段的数据包大小削减至67个字符。为了避免意外的电信账单惊喜，在运行批量营销活动之前，请务必使用在线短信字符计数器来监控您的活动编码标签！',
      },
      {
        q: '短信中的特殊字符和表情符号算作一个字符吗？',
        a: '不，并非如此。虽然标准字母或数字算作单个字符，但特殊符号和表情符号在全球电信网络中的处理方式截然不同。属于标准GSM基本扩展表的特殊符号——例如欧元符号（€）、方括号[ ]、花括号{ }和竖线符号|——实际上每个算作2个字符，尽管它们使您的消息保持在高效的GSM 7位模式下。另一方面，表情符号是高度复杂的数据包，无法放入标准的7位文本中。添加一个表情符号会将您的消息强制转换为16位Unicode，将每段的消息总量从160个字符压缩到70个字符。',
      },
    ],
  },
    ...
    about: {
      ...
      sections: [
        ...
        },
        {
          heading: '以隐私优先打造',
          paragraphs: [
            '一切都在你的浏览器中运行。你的文字永远不会被上传，没有账户，而且本工具免费使用。它靠预留位置中的不打扰广告来支持，这些广告绝不会移动页面布局。完整细节请阅读我们的<strong>隐私政策</strong>。',
          ],
        },
      ],
    },
